...
const store={data:null,view:"overview",sort:"recommended",filter:"all",search:"",busy:false};
const esc=v=>String(v??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
const chip=(t,c="")=>`<span class="chip ${c}">${esc(t)}</span>`;
const metric=(l,v,n="")=>`<div class="card"><div class="metric">${esc(l)}</div><div class="big">${esc(v)}</div><div class="muted">${esc(n)}</div></div>`;
function itemCard(item,{equip=false,learn=false}={}){const tags=[];if(item.equipped)tags.push(chip("Equipped","good"));if(item.recommended)tags.push(chip("Suggested","warn"));if(item.crafted)tags.push(chip("Crafted"));if(item.recipe_code)tags.push(chip(item.recipe_code));if(item.quantity>1)tags.push(chip(`x${item.quantity}`));const actions=[];if(equip&&item.can_equip)actions.push(`<button ${item.equipped?"disabled":""} data-action="equip" data-index="${item.index}">${item.equipped?"Equipped":"Equip now"}</button>`);if(learn&&item.can_learn)actions.push(`<button data-action="learn" data-index="${item.index}">Learn</button>`);return `<div class="item"><div><div class="row"><strong>${esc(item.name)}</strong>${tags.join("")}</div><div class="row">${chip(`${item.type} · ${item.rarity}`)}${chip(`P ${item.power}`)}${chip(`Q ${item.quality}`)}${item.slot?chip(item.slot_label):""}${item.score?chip(`Score ${item.score}`):""}</div><div class="muted">${esc(item.summary)}</div></div><div class="actions">${actions.join("")}</div></div>`}
...
function gear(d){const slots=d.equipment.map(slot=>`<div class="card"><div class="metric">${esc(slot.label)}</div>${slot.current?itemCard(slot.current):`<div class="empty">Nothing equipped in ${esc(slot.label)}.</div>`}${slot.has_upgrade&&slot.recommended?`<div class="card"><div class="metric">Suggested Upgrade</div><strong>${esc(slot.recommended.name)}</strong><div class="muted">${esc(slot.recommended.summary)}</div><button data-action="equip" data-index="${slot.recommended.index}">Equip suggested now</button></div>`:`<div class="muted">Current item is already the best known pick.</div>`}</div>`).join("");return `<div class="cards"><section class="panel"><div class="head"><h3>Equipped Slots</h3><div class="status">${chip(`${d.inventory_summary.equipped_count} equipped`)}${chip(`${d.hero.hero_power} hero power`)}</div></div><div class="body"><div class="grid3">${slots}</div><div class="row"><button data-action="autoequip">Equip best known set</button><button class="secondary" data-action="refresh">Refresh after manual checks</button></div></div></section><section class="panel"><div class="head"><h3>Inventory</h3><div class="toolbar"><select id="inventory-filter"><option value="all">All items</option><option value="equippable">Equippable</option><option value="equipped">Equipped only</option><option value="recipes">Blueprints</option><option value="materials">Materials</option></select><select id="inventory-sort"><option value="recommended">Suggested first</option><option value="score">Highest score</option><option value="power">Highest power</option><option value="quality">Highest quality</option><option value="rarity">Highest rarity</option><option value="name">Name</option><option value="newest">Newest first</option></select><input id="inventory-search" placeholder="Search item name"></div></div><div id="inventory-panel" class="body"></div></section></div>`}
function crafting(d){const recipes=d.recipes.known.length?d.recipes.known.map(r=>`<div class="card"><div class="row"><strong>${esc(r.name)}</strong>${chip(r.code)}${r.can_craft?chip("Craftable","good"):chip("Blocked","warn")}</div><div class="row">${chip(`${r.materials} materials`)}${chip(`${r.gold} gold`)}</div><div class="muted">${esc(r.status)}</div><button ${r.can_craft?"":"disabled"} data-action="craft" data-recipe="${esc(r.code)}">Craft now</button></div>`).join(""):`<div class="empty">No learned recipes yet.</div>`;const drops=d.recipes.blueprints.length?d.recipes.blueprints.map(i=>itemCard(i,{learn:true})).join(""):`<div class="empty">No blueprint drops waiting.</div>`;return `<div class="cards"><section class="panel"><div class="head"><h3>Crafting Overview</h3><div class="status">${chip(`${d.inventory_summary.materials} materials`)}${chip(`${d.hero.gold} gold`)}${chip(`${d.recipes.known.length} known recipes`)}</div></div><div class="body"><div class="grid3">${metric("Materials",d.inventory_summary.materials,"Salvage and dungeon drops")}${metric("Gold",d.hero.gold,"Used by crafting and upgrades")}${metric("Blueprints",d.inventory_summary.blueprints,"Unlearned recipe drops")}</div><div class="row"><button data-action="forge">Forge / Salvage step</button></div></div></section><section class="panel"><div class="head"><h3>Craftable Recipes</h3><div class="status">${chip(`${d.recipes.known.filter(r=>r.can_craft).length} ready`,d.recipes.known.some(r=>r.can_craft)?"good":"")}</div></div><div class="body"><div class="grid2">${recipes}</div></div></section><section class="panel"><div class="head"><h3>Blueprint Drops</h3><div class="status">${chip(`${d.recipes.blueprints.length} waiting`,d.recipes.blueprints.length?"warn":"")}</div></div><div class="body"><div class="grid2">${drops}</div></div></section></div>`}
...
function notice(msg,t="good"){const n=document.getElementById("notice");if(!n)return;n.textContent=msg;n.className=`notice visible ${t}`;clearTimeout(notice.timer);notice.timer=setTimeout(()=>n.className="notice",3200)}
async function fetchState(silent=false){if(!silent)store.busy=true;const r=await fetch("/api/manager-state");store.data=await r.json();render();store.busy=false}
async function run(action,payload={}){if(store.busy)return;store.busy=true;try{const map={equip:"gear",autoequip:"gear",forge:"crafting",learn:"crafting",craft:"crafting",perk:"progression",stat:"progression"};const r=await fetch("/api/action",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,...payload})});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.message||`Action failed: ${action}`);store.data=data.state;if(map[action])store.view=map[action];render();notice(data.message||"Action completed.");}catch(e){notice(e.message||"Action failed.","bad")}finally{store.busy=false}}
