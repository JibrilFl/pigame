...
.app{min-height:100vh;display:grid;grid-template-columns:280px 1fr}.side{background:linear-gradient(180deg,#f6efdf,#ecdfc8);border-right:1px solid var(--line);padding:18px;display:grid;gap:16px;align-content:start;position:sticky;top:0;height:100vh}.brand{padding:14px 16px;border-radius:20px;background:linear-gradient(135deg,var(--brand),var(--brand2));color:#f7fbff;box-shadow:var(--shadow)}.brand h1{margin:0;font:700 30px Georgia,"Times New Roman",serif}.brand p{margin:6px 0 0;font-size:13px;opacity:.88}
.nav{display:grid;gap:8px}.nav button{width:100%;justify-content:flex-start;text-align:left;background:#fff9ef;color:var(--ink);border:1px solid var(--line);box-shadow:none}.nav button.active{background:linear-gradient(135deg,var(--brand),var(--brand2));color:#fff;border-color:transparent}.side .meta{display:grid;gap:8px}.chip,.pill{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:6px 10px;font-size:12px;white-space:nowrap;border:1px solid var(--line);background:#fdf8ef}.good{color:var(--good)!important}.warn{color:var(--warn)!important}.bad{color:var(--bad)!important}
.main{padding:18px;display:grid;gap:16px}.notice{padding:12px 14px;border-radius:14px;border:1px solid var(--line);background:rgba(255,250,241,.92);display:none;box-shadow:var(--shadow)}.notice.visible{display:block}.top{display:flex;justify-content:space-between;gap:12px;align-items:center;background:linear-gradient(135deg,#31251b,#6e4428);color:#fff8ef;border-radius:22px;padding:18px 20px;box-shadow:var(--shadow)}.top h2{margin:0;font:700 28px Georgia,"Times New Roman",serif}.row,.toolbar,.actions,.status,.hero-tags{display:flex;flex-wrap:wrap;gap:8px}
...
<script>
const VIEWS=[["overview","Overview"],["gear","Gear"],["crafting","Crafting"],["progression","Progression"],["activity","Activity"]];
const store={data:null,view:"overview",sort:"recommended",filter:"all",search:"",busy:false,flash:null};
const esc=v=>String(v??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
const chip=(t,c="")=>`<span class="chip ${c}">${esc(t)}</span>`;
const metric=(l,v,n="")=>`<div class="card"><div class="metric">${esc(l)}</div><div class="big">${esc(v)}</div><div class="muted">${esc(n)}</div></div>`;
function itemCard(item,{equip=false,learn=false}={}){const tags=[];if(item.equipped)tags.push(chip("Equipped","good"));if(item.recommended)tags.push(chip("Suggested","warn"));if(item.crafted)tags.push(chip("Crafted"));if(item.recipe_code)tags.push(chip(item.recipe_code));if(item.quantity>1)tags.push(chip(`x${item.quantity}`));const actions=[];if(equip&&item.can_equip)actions.push(`<button ${item.equipped?"disabled":""} data-action="equip" data-index="${item.index}">${item.equipped?"Equipped":"Equip now"}</button>`);if(learn&&item.can_learn)actions.push(`<button data-action="learn" data-index="${item.index}">Learn</button>`);const focused=store.flash&&store.flash.index===item.index;return `<div class="item ${focused?"focus":""}"><div><div class="row"><strong>${esc(item.name)}</strong>${tags.join("")}</div><div class="row">${chip(`${item.type} · ${item.rarity}`)}${chip(`P ${item.power}`)}${chip(`Q ${item.quality}`)}${item.slot?chip(item.slot_label):""}${item.score?chip(`Score ${item.score}`):""}</div><div class="muted">${esc(item.summary)}</div></div><div class="actions">${actions.join("")}</div></div>`}
function renderSidebar(){const side=document.getElementById("sidebar"),meta=document.getElementById("side-meta"),d=store.data;if(!side||!meta||!d)return;side.innerHTML=VIEWS.map(([id,label])=>`<button class="${store.view===id?"active":""}" data-view="${id}">${label}</button>`).join("");meta.innerHTML=`<div class="card"><div class="metric">Hero</div><strong>${esc(d.hero.name)}</strong><div class="row">${chip(d.hero.specialization)}${chip(`Lv ${d.hero.level}`)}${chip(`Pwr ${d.hero.hero_power}`)}</div></div><div class="card"><div class="metric">Needs You</div><div class="row">${chip(`${d.hero.unspent_stat_points} stat pts`,d.hero.unspent_stat_points?"warn":"")}${chip(`${d.hero.perk_points} perk pts`,d.hero.perk_points?"warn":"")}${chip(`${d.inventory_summary.blueprints} blueprints`,d.inventory_summary.blueprints?"warn":"")}</div></div>`}
function renderTopbar(){const top=document.getElementById("topbar"),d=store.data;if(!top||!d)return;top.innerHTML=`<div><h2>${esc(VIEWS.find(v=>v[0]===store.view)?.[1]||"Manager")}</h2><div class="hero-tags">${chip(d.hero.title)}${chip(d.hero.activity)}${chip(d.hero.awaiting_text,d.hero.awaiting_player?"warn":"good")}</div></div><div class="row"><button class="secondary" data-action="refresh">Refresh</button><button class="secondary" data-action="tickQuick" data-ticks="1">Tick 1</button><button class="secondary" data-action="tickQuick" data-ticks="5">Tick 5</button><button class="secondary" data-action="tickQuick" data-ticks="15">Tick 15</button></div>`}
function overview(d){const steps=d.next_steps.length?d.next_steps.map(s=>`<div class="item"><div><div class="row">${chip(s.tone==="good"?"Ready":"Attention",s.tone||"warn")}<strong>${esc(s.title)}</strong></div><div class="muted">${esc(s.detail)}</div></div></div>`).join(""):`<div class="empty">No urgent interventions.</div>`;return `<div class="cards"><section class="panel"><div class="head"><h3>Hero Snapshot</h3><div class="status">${chip(`Loss ${d.hero.loss_streak}`,d.hero.loss_streak>=2?"bad":"")}${chip(`Bosses ${d.hero.bosses_defeated}`)}${chip(d.world.boss_status,d.world.boss_active?"bad":"")}</div></div><div class="body"><div class="muted">${esc(d.hero.summary)}</div><div class="grid4">${metric("Level",d.hero.level,`${d.hero.experience} XP`)}${metric("Hero Power",d.hero.hero_power,"Estimated combat score")}${metric("Gold",d.hero.gold,`${d.hero.supplies} supplies`)}${metric("Depth",d.hero.depth,`${d.hero.wins}W / ${d.hero.losses}L`)}</div><div class="hero-grid">${metric("Power",d.hero.stats.power,`Total ${d.hero.total_stats.power}`)}${metric("Vitality",d.hero.stats.vitality,`Total ${d.hero.total_stats.vitality}`)}${metric("Agility",d.hero.stats.agility,`Total ${d.hero.total_stats.agility}`)}${metric("Insight",d.hero.stats.insight,`Total ${d.hero.total_stats.insight}`)}${metric("Luck",d.hero.stats.luck,`Total ${d.hero.total_stats.luck}`)}</div></div></section><section class="panel"><div class="head"><h3>What Needs You</h3><div class="status">${chip(`${d.recipes.known.length} recipes`)}${chip(`${d.inventory_summary.blueprints} blueprints`,d.inventory_summary.blueprints?"warn":"")}</div></div><div class="body">${steps}</div></section><section class="panel"><div class="head"><h3>World and Device</h3><div class="status">${chip(d.world.region)}${chip(d.world.threat)}</div></div><div class="body"><div class="grid2">${metric("Biome Tier",d.world.biome_tier,`Danger ${d.world.danger_rating}`)}${metric("Battery",d.device.battery_percent??"?",d.device.battery_detail)}</div><pre class="preview">${esc(d.screen_preview)}</pre></div></section></div>`}
function gear(d){const slots=d.equipment.map(slot=>{const current=slot.current?itemCard(slot.current):`<div class="empty">Nothing equipped in ${esc(slot.label)}.</div>`;let compare=`<div class="muted">Current item is already the best known pick.</div>`;if(slot.has_upgrade&&slot.recommended){const delta=slot.score_delta||0;compare=`<div class="compare"><div class="card"><div class="metric">Current</div>${slot.current?`<strong>${esc(slot.current.name)}</strong><div class="muted">${esc(slot.current.summary)}</div><div class="row">${chip(`Score ${slot.current.score||0}`)}</div>`:`<div class="muted">Empty slot</div>`}</div><div class="card ${store.flash&&store.flash.index===slot.recommended.index?"focus":""}"><div class="metric">Suggested</div><strong>${esc(slot.recommended.name)}</strong><div class="muted">${esc(slot.recommended.summary)}</div><div class="row">${chip(`Score ${slot.recommended.score||0}`)}<span class="delta ${delta>=0?"up":"down"}">${delta>=0?"+":""}${delta}</span></div><button data-action="equip" data-index="${slot.recommended.index}">Equip suggested now</button></div></div>`}return `<div class="card ${store.flash&&store.flash.slot===slot.slot?"focus":""}"><div class="metric">${esc(slot.label)}</div>${current}${compare}</div>`}).join("");const equippedNow=store.flash&&store.flash.action==="equip"?`<div class="card focus"><div class="metric">Last equipment action</div><strong>${esc(store.flash.name||"Item equipped")}</strong><div class="muted">The slot and inventory list were refreshed from live save state.</div></div>`:"";return `<div class="cards"><section class="panel"><div class="head"><h3>Equipped Slots</h3><div class="status">${chip(`${d.inventory_summary.equipped_count} equipped`)}${chip(`${d.hero.hero_power} hero power`)}</div></div><div class="body">${equippedNow}<div class="grid3">${slots}</div><div class="row"><button data-action="autoequip">Equip best known set</button><button class="secondary" data-action="refresh">Refresh after manual checks</button></div></div></section><section class="panel"><div class="head"><h3>Inventory</h3><div class="toolbar"><select id="inventory-filter"><option value="all">All items</option><option value="equippable">Equippable</option><option value="equipped">Equipped only</option><option value="recipes">Blueprints</option><option value="materials">Materials</option></select><select id="inventory-sort"><option value="recommended">Suggested first</option><option value="score">Highest score</option><option value="power">Highest power</option><option value="quality">Highest quality</option><option value="rarity">Highest rarity</option><option value="name">Name</option><option value="newest">Newest first</option></select><input id="inventory-search" placeholder="Search item name"></div></div><div id="inventory-panel" class="body"></div></section></div>`}
function crafting(d){const recipes=d.recipes.known.length?d.recipes.known.map(r=>`<div class="card ${store.flash&&store.flash.recipe_code===r.code?"focus":""}"><div class="row"><strong>${esc(r.name)}</strong>${chip(r.code)}${r.can_craft?chip("Craftable","good"):chip("Blocked","warn")}</div><div class="row">${chip(`${r.materials} materials`)}${chip(`${r.gold} gold`)}${chip(`${r.craftable_count}x now`,r.craftable_count?"good":"")}</div><div class="muted">${esc(r.status)}</div><div class="row">${r.missing_materials?chip(`Missing ${r.missing_materials} mats`,"warn"):""}${r.missing_gold?chip(`Missing ${r.missing_gold} gold`,"warn"):""}</div><button ${r.can_craft?"":"disabled"} data-action="craft" data-recipe="${esc(r.code)}">Craft now</button></div>`).join(""):`<div class="empty">No learned recipes yet.</div>`;const drops=d.recipes.blueprints.length?d.recipes.blueprints.map(i=>itemCard(i,{learn:true})).join(""):`<div class="empty">No blueprint drops waiting.</div>`;const readyCount=d.recipes.known.filter(r=>r.can_craft).length;return `<div class="cards"><section class="panel"><div class="head"><h3>Crafting Overview</h3><div class="status">${chip(`${d.inventory_summary.materials} materials`)}${chip(`${d.hero.gold} gold`)}${chip(`${d.recipes.known.length} known recipes`)}</div></div><div class="body"><div class="grid4">${metric("Materials",d.inventory_summary.materials,"Salvage and dungeon drops")}${metric("Gold",d.hero.gold,"Used by crafting and upgrades")}${metric("Blueprints",d.inventory_summary.blueprints,"Unlearned recipe drops")}${metric("Ready Recipes",readyCount,"Can be crafted right now")}</div><div class="row"><button data-action="forge">Forge / Salvage step</button><button class="secondary" data-action="refresh">Refresh materials</button></div></div></section><section class="panel"><div class="head"><h3>Craftable Recipes</h3><div class="status">${chip(`${readyCount} ready`,readyCount?"good":"")}</div></div><div class="body"><div class="grid2">${recipes}</div></div></section><section class="panel"><div class="head"><h3>Blueprint Drops</h3><div class="status">${chip(`${d.recipes.blueprints.length} waiting`,d.recipes.blueprints.length?"warn":"")}</div></div><div class="body"><div class="grid2">${drops}</div></div></section></div>`}
function progression(d){const statButtons=Object.entries(d.hero.stats).map(([k,v])=>`<div class="card"><div class="metric">${esc(k)}</div><div class="big">${esc(v)}</div><button ${d.hero.unspent_stat_points>0?"":"disabled"} data-action="stat" data-stat="${esc(k)}">Spend 1 point</button></div>`).join("");const learned=d.perks.learned.length?d.perks.learned.map(p=>`<div class="card"><strong>${esc(p.name)}</strong><div class="muted">${esc(p.description)}</div></div>`).join(""):`<div class="empty">No perks learned yet.</div>`;const available=d.perks.available.length?d.perks.available.map(p=>`<div class="card"><div class="metric">${esc(p.code)}</div><strong>${esc(p.name)}</strong><div class="muted">${esc(p.description)}</div><button ${d.hero.perk_points>0?"":"disabled"} data-action="perk" data-perk="${esc(p.code)}">Learn perk</button></div>`).join(""):`<div class="empty">No perk choices available right now.</div>`;return `<div class="cards"><section class="panel"><div class="head"><h3>Allocation</h3><div class="status">${chip(`${d.hero.unspent_stat_points} stat pts`,d.hero.unspent_stat_points?"warn":"")}${chip(`${d.hero.perk_points} perk pts`,d.hero.perk_points?"warn":"")}</div></div><div class="body"><div class="grid3"><div class="card"><div class="metric">Specialization</div><select id="specialization-select">${d.specializations.map(s=>`<option value="${esc(s)}" ${s===d.hero.specialization?"selected":""}>${esc(s)}</option>`).join("")}</select><button data-action="specialization">Apply specialization</button></div><div class="card"><div class="metric">Manual Time Advance</div><input id="ticks-input" type="number" min="1" max="50" value="3"><button data-action="tick">Run ticks</button></div><div class="card"><div class="metric">Camp Hold</div><div class="muted">${esc(d.hero.awaiting_text)}</div><button ${d.hero.awaiting_player?"":"disabled"} data-action="resume">Resume autonomous runs</button></div></div><div class="hero-grid">${statButtons}</div></div></section><section class="panel"><div class="head"><h3>Perks</h3><div class="status">${chip(`${d.perks.learned.length} learned`)}${chip(`${d.perks.available.length} available`)}</div></div><div class="body"><div class="grid2">${learned}</div><div class="grid2">${available}</div></div></section></div>`}
function activity(d){const log=d.activity_log.length?d.activity_log.map(line=>`<div class="card"><div class="muted">${esc(line)}</div></div>`).join(""):`<div class="empty">No recent activity yet.</div>`;return `<div class="cards"><section class="panel"><div class="head"><h3>Recent Activity</h3><div class="status">${chip(d.world.last_event)}</div></div><div class="body">${log}</div></section><section class="panel"><div class="head"><h3>Device Preview</h3><div class="status">${chip(d.device.battery_text,d.device.low_power_mode?"warn":"")}</div></div><div class="body"><pre class="preview">${esc(d.screen_preview)}</pre></div></section><section class="panel"><div class="head"><h3>Developer Tools</h3><div class="status">${chip("debug only")}</div></div><div class="body"><div class="grid3"><div class="card"><div class="metric">Add Supplies</div><input id="dev-supplies" type="number" value="5"><button data-action="supplies">Apply</button></div><div class="card"><div class="metric">Add Gold</div><input id="dev-gold" type="number" value="100"><button data-action="gold">Apply</button></div><div class="card"><div class="metric">Add XP</div><input id="dev-xp" type="number" value="50"><button data-action="xp">Apply</button></div></div><pre class="raw">${esc(JSON.stringify(d.raw_state,null,2))}</pre></div></section></div>`}
function match(item){if(store.filter==="equippable"&&!item.can_equip)return false;if(store.filter==="equipped"&&!item.equipped)return false;if(store.filter==="recipes"&&item.type!=="recipe")return false;if(store.filter==="materials"&&item.type!=="material")return false;if(store.search&&!item.name.toLowerCase().includes(store.search.toLowerCase()))return false;return true}
function sortItems(items){const rr={common:0,uncommon:1,rare:2,epic:3,mythic:4};const list=[...items].filter(match);list.sort((a,b)=>{if(store.sort==="name")return a.name.localeCompare(b.name);if(store.sort==="power")return b.power-a.power||b.quality-a.quality;if(store.sort==="quality")return b.quality-a.quality||b.power-a.power;if(store.sort==="rarity")return (rr[b.rarity]??0)-(rr[a.rarity]??0)||b.power-a.power;if(store.sort==="score")return (b.score??0)-(a.score??0)||b.power-a.power;if(store.sort==="newest")return b.index-a.index;return Number(b.recommended)-Number(a.recommended)||Number(b.equipped)-Number(a.equipped)||(b.score??0)-(a.score??0)});return list}
...
document.addEventListener("click",async e=>{const v=e.target.closest("[data-view]");if(v){store.view=v.dataset.view;render();return}const b=e.target.closest("[data-action]");if(!b)return;const a=b.dataset.action;if(a==="refresh"){await fetchState(true);notice("State refreshed.");return}if(a==="equip"){await run("equip",{index:Number(b.dataset.index)});return}if(a==="learn"){await run("learn",{index:Number(b.dataset.index)});return}if(a==="craft"){await run("craft",{recipe_code:b.dataset.recipe});return}if(a==="perk"){await run("perk",{perk_code:b.dataset.perk});return}if(a==="stat"){await run("stat",{stat:b.dataset.stat});return}if(a==="tickQuick"){await run("tick",{ticks:Number(b.dataset.ticks||"1")});return}if(a==="tick"){await run("tick",{ticks:Number(document.getElementById("ticks-input")?.value||"1")});return}if(a==="specialization"){await run("specialization",{specialization:document.getElementById("specialization-select")?.value||""});return}if(a==="supplies"||a==="gold"||a==="xp"){await run(a,{amount:Number(document.getElementById(`dev-${a}`)?.value||"0")});return}await run(a)});
fetchState().catch(e=>notice(e.message||"Failed to load manager.","bad"));
</script></body></html>"""
